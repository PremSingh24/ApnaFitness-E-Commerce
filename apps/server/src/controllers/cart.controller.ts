...
export const addItemToCartHandler = async(req:Request, res:Response) =>{
  ...
  try{
    ...
    if(Product){
      ...
      if(user){
        const ItemPresent = user.cart.find((cartItem)=> cartItem.item.equals(Product._id));

        if(!ItemPresent){
          //await user.updateOne({$push:{cart:{item:Product}}});
          ...
          res.status(201).json({ message: "Product Added to Cart successfully",cartId });
...
export const removeItemFromCartHandler  = async(req:Request, res:Response) =>{
...
export const updateCartItemHandler   = async(req:Request, res:Response) =>{
  ...
  try{
    ...
    if(user){
      ...
      if(itemPresent){
        await user.updateOne({$set:{"cart.$[item].quantity":req.body.quantity}},{arrayFilters: [{"item._id": req.params.CartId}]});
