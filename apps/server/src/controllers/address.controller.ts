...
export const getAddressHandler = async(req:Request, res:Response) => {
    ...
    try{
        ...
        if(user){
            ...
            res.status(401).json({message:"Something Went Wrong!, Try to Login Again"});
        }

    }catch(error){

        res.status(409).json({message:"Can not Get Address!, Try Again Later ",error});
    }
...
export const addAddressHandler = async(req:Request, res:Response)=>{ //Make the user Add only 5 address at maximum 
    ...
    if(validAddress.success){
        ...
        try{
            ...
            if(user){
                ...
                await user.updateOne({$set:{"address.$[item].name":validAddress.data.name,"address.$[item].mobile":validAddress.data.mobile,
                "address.$[item].street":validAddress.data.street,"address.$[item].city":validAddress.data.city,"address.$[item].state":validAddress.data.state,
                "address.$[item].country":validAddress.data.country,"address.$[item].pincode":validAddress.data.pincode}},
...
export const removeAddressHandler = async(req:Request, res:Response)=>{
    try{
        ...
        if(user){
            const removeResponse = await user.updateOne({$pull:{address:{_id:req.params.AddressId}}});
