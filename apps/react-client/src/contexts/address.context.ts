...
const useAddressStore = create<addressState>((set) => ({
    ...
    addAdress: (newAddress) => set((state)=>({address:[...state.address,newAddress]})),
    updateAddress: (_id,updatedAddress) => set((state)=>({address:state.address.map(address=>address._id===_id ? ({...address._id,updatedAddress}):address)})),
    removeAddress:(_id) => set((state)=>({address:state.address.filter((address)=>address._id !== _id)}))
